...
def affine_backward(dout, cache):
    ...
    dx = np.dot(dout,w.T).reshape(x.shape)      # reshape(N,D)
...
#         v3 = v1 - v2            # NxD
#         v4 = v3 ** 2            # NxD
#         v5 = np.mean(v4,axis=0) # 1xD
#         v6 = np.sqrt(v5+eps)    # 1xD
#         v7 = 1 / v6             # 1xD
#         v8 = v3 * v7            # NxD
#         v9 = v8 * gamma         # NxD
#         v10 = v9 + beta         # NxD
...
# =============================================================================
#     #dbatch_norm_x = np.sum(dout,axis=0) * gamma        # 1xD  不能这样算
#     # u = x - x_mean
#     # v = sqrt(x_var+eps)
#     
#     u = x - batch_mean
#     v = np.sqrt(batch_var + eps)
#     
#     du = np.ones((N,D)) * (1 - 1/N)                    # NxD
#     dv = 1 / v * (N-1) * (x - batch_mean) / N**2
#     
#     dx = dout * (du*v - u*dv) / v**2
# =============================================================================
    
# =============================================================================
#     #gamma, x, u_b, sigma_squared_b, eps, x_hat = cache
#     (x, x_hat, u_b, sigma_squared_b, gamma, beta, bn_param) = cache
#     dx_1 = gamma * dout
...
# =============================================================================
    ...
    # according to the computing graph
    v1 = x                  # NxD
    v2 = batch_mean         # 1xD
    v3 = v1 - v2            # NxD
    v4 = v3 ** 2            # NxD
    ...
    dv8 = dout * gamma                  # NxD
    dv7 = np.sum(dv8 * v3,axis=0)       # 1xD
    dv3 = dv8 * v7                      # NxD
    dv6 = dv7 * (-1.0 / v6**2)            # 1xD
    dv5 = dv6 * (0.5 * (v5 + eps) ** (-0.5))     # 1xD  
    dv4 = dv5 * np.ones_like(v4) * (1.0 / float(N))     # NxD
    dv3 += dv4 * 2 * v3                         # NxD
    dv1 = dv3
    ...
    dx += 1 / float(N) * np.ones((v3.shape)) * dv2
    
    
    dbeta = np.sum(dout,axis=0)
    dgamma = np.sum(dout * v8,axis=0)
# =============================================================================
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = np.sum(dout,axis=0)
    dgamma = np.sum(dout * batch_norm_x,axis=0)
    dx = (1. / N) * gamma * (batch_var + eps)**(-1. / 2.) \
         * (N * dout - np.sum(dout, axis=0) - (x - batch_mean) 
         * (batch_var + eps)**(-1.0) 
         * np.sum(dout * (x -batch_mean), axis=0))
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    dx[n,c,h_out*stride:h_out*stride+HH,w_out*stride:w_out*stride+WW] = dout[n,c,h_out,w_out] * (x[n,c,h_out*stride:h_out*stride+HH,w_out*stride:w_out*stride+WW] == np.max(x[n,c,h_out*stride:h_out*stride+HH,w_out*stride:w_out*stride+WW]))
