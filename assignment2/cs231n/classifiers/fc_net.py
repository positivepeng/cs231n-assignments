...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_out = affine_forward(h, self.params['W2'], self.params['b2'])
        ...
        dh, grads['W2'], grads['b2'] = affine_backward(dout, cache_out)
        _, grads['W1'], grads['b1'] = affine_relu_backward(dh, cache_h)
    
        # add regularizaiton term
...
# =============================================================================
#         # get grads from strach
#         grads["W1"] = X.T.dot(dscores.dot(W2.T)) + W1 * self.reg
#         grads["b1"] = np.sum(dscores.dot(W2.T),axis=0)
#         grads["W2"] = (X.dot(W1)+b1).T.dot(dscores) + W2 * self.reg
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if self.use_batchnorm:
            for dim_id,hidden_dim in enumerate(hidden_dims):
                self.params["gamma"+str(dim_id+1)] = np.ones([hidden_dim])
                self.params["beta"+str(dim_id+1)] = np.zeros([hidden_dim])
...
# =============================================================================
    ...
    def loss(self, X, y=None):
        ...
        cache = {}
        # (1) forward prop: input-to-hidden
        if self.use_batchnorm:
            h, cache['h1'] = affine_batchnorm_relu_forward(X, self.params['W1'], self.params['b1'], self.params["gamma1"], self.params["beta1"])
        else:
        ...
        # (1) compute loss and backprop softmax layer 
        loss, dout = softmax_loss(scores, y)
        ...
        for i in reversed(range(self.num_layers - 1)):
            ...
            if self.use_batchnorm:
                ...
                dh ,grads['W%d' %(i+1)], grads['b%d' %(i+1)] = affine_relu_backward(dh, cache['h%d' %(i+1)])
...
def affine_batchnorm_relu_forward(x, W, b, gamma, beta):
    ...
    cache = (affine_out,affine_cache,batchnorm_out,batchnorm_cache,relu_cache)
    return out,cache

def affine_batchnorm_relu_backward(dout,cache):
    affine_out,affine_cache,batchnorm_out,batchnorm_cache,relu_cache = cache
