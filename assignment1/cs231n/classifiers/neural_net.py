...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # forward
        FC1_output = X.dot(W1) + b1.T     # (N,H)
        Relu_output = np.maximum(FC1_output,0)   
        FC2_output = (Relu_output.dot(W2) + b2.T)
        scores = FC2_output
        ...
        dh = dout.dot(W2.T) 
        dz = dout.dot(W2.T) * (FC1_output > 0)
        
        # compute gradient for parameters
        grads['W2'] = np.dot(Relu_output.T, dout) / N      # (H, C)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            batch_indexs = np.random.choice(np.arange(num_train),batch_size)
    ...
    def predict(self, X):
        ...
        Relu_output = np.maximum(X.dot(self.params["W1"])+self.params["b1"],0)
